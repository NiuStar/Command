...
func (buf *buffer) ReadAll() ([]byte, error) {
	//nread := 0
	if buf.b.Len() > 0 {
		return buf.b.Bytes(),nil
	}
	chunk := make([]byte,1024)
	fn, err := buf.f.Read(chunk)
	return chunk[:fn], err
}
...
func CommandCreate(command string) (*ExpectSubprocess, error) {
	expect, err := _spawn(command)
	if err != nil {
		return nil, err
	}
...
func (expect *ExpectSubprocess) ReadAll() (string, error) {
	result,err := expect.buf.ReadAll()
...
func _spawn(command string) (*ExpectSubprocess, error) {
	...
	wrapper.Cmd = exec.Command("/bin/bash", "-c",command)
